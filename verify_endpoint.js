...
async function testEndpoint() {
    ...
    await db.run("DELETE FROM invoices WHERE id IN (9001, 9002)");
    await db.run(`INSERT INTO invoices (company_id, week_start, week_end, total_cents, status, receipt_url, created_at, id) VALUES (?, '2023-11-01', '2023-11-07', 15000, 'charged', 'https://receipt.stripe.com/1', ?, 9001)`, emp1.id, nowIso);
    await db.run(`INSERT INTO invoices (company_id, week_start, week_end, total_cents, status, receipt_url, created_at, id) VALUES (?, '2023-11-01', '2023-11-07', 25000, 'charged', 'https://receipt.stripe.com/2', ?, 9002)`, emp2.id, nowIso);
