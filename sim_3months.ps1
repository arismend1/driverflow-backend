...
$LOG_FILE=Join-Path $LOG_DIR ("sim_3months_"+(Get-Date -Format "yyyyMMdd_HHmmss")+".log")

New-Item -ItemType Directory -Force -Path $LOG_DIR | Out-Null
Set-Location $PROJECT

"=== SIM 3 MONTHS START: $(Get-Date) ===" | Out-File $LOG_FILE -Encoding utf8
...
# ===== 12 WEEKS (3 MONTHS) =====
# Use different company IDs to avoid "active request" / availability constraints
$companies = 777..788

for ($i=0; $i -lt 12; $i++) {
  "`n==================== WEEK " + ($i+1) + " / 12 ====================" | Out-File $LOG_FILE -Append -Encoding utf8
  Run-Week $companies[$i]
...
"`n=== SIM 3 MONTHS DONE: $(Get-Date) ===" | Out-File $LOG_FILE -Append -Encoding utf8
