...
app.post('/register', async (req, res) => {
    ...
    if (type === 'empresa') {
        ...
        const registerTx = db.transaction(() => {
            ...
            // We use 'ciudad' for simple city, but also store full address fields if schema supports. 
            // Migration added: address_line1, address_state. 'ciudad' exists.
            ...
            // match_prefs: { req_license, req_experience, req_team_driving, req_start, req_restrictions }
        ...
        try {
            ...
            return res.status(201).json({ id: result.id, type: 'empresa', message: 'Empresa registrada. Activar búsqueda cuando esté lista.' });
        } catch (err) {
            if (err.code === 'SQLITE_CONSTRAINT_UNIQUE') return res.status(409).json({ error: 'Contacto ya registrado' });
            return res.status(500).json({ error: err.message });
        }
...
app.post('/login', async (req, res) => {
    ...
    if (await bcrypt.compare(password, row.password_hash)) {
        // Token incluye ID, Tipo y Licencia (si es driver) para facilitar lógica
        const payload = {
            id: row.id,
            type: type,
